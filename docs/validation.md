...
**The interval `auclast` of PKNCA.** The PKNCA vignette prints `auclast` of 92.365442 for subject 1 over the interval 0 to 24 h, where `pkpdutils` computes 146.01 with `partial_auc(batch, 0.0, 24.0)`. The two answer different questions: PKNCA sums the trapezoids between the observations that fall inside the interval, and the last observation of subject 1 before 24 h is at 12.12 h, so its number is the area from 0 to 12.12 h; `partial_auc` interpolates the concentration at the end of the interval and integrates to 24 h. Neither is wrong and neither is comparable to the other, so the number is not part of the reference file.
