...
## Breaking changes against 1.0.0

The dosing protocols change the data model and the multiple dose parameters of the non-compartmental analysis; a release note is written by the maintainer with the version.
...
- `superposition(timecourse, dosing)` takes a `Dosing` protocol or a `DosingRegimen`, no longer a `DosingRegimen` alone; the predicted curve carries the protocol and no label (the label of the single dose curve describes that curve).
- `Timecourse.dose` is a read-only property, the first dose of `Timecourse.dosing`. `model_copy(update={"dose": ...})` is a silent no-op, since `dose` is not a field: the protocol is replaced with `model_copy(update={"dosing": Dosing.single(dose)})`.
- A `Timecourses` dataset carries the dose dimension `dose_index`: `dose_amount`, `dose_time` and `dose_duration` are 2-D over `(*sample_dims, dose_index)`. The 1-D view of a single dose batch is `first_dose_amount`/`first_dose_time` and `last_dose_amount`/`last_dose_time`.
- A batch with a sample dimension named `dose_index` raises when it carries doses, since that name is the dose dimension.
- A multiple dose analysis (a protocol of more than one dose, or `NCAOptions.tau`) reports `cl`, `cl_f`, `vz`, `vz_f`, `vss`, `auc_inf_dn` and `cmax_dn` as `NaN`: the slice after the last dose carries the exposure of the earlier doses as well and is not a single dose curve. `auc_inf_obs`, `auc_inf_pred`, `aumc_inf` and `mrt` stay and describe the decline after the last dose.
- The clearance at steady state of an extravascular route is reported as `cl_ss_f`, not as `cl_ss`, the way `compute_parameters` reports `cl_f`.
- `read_events` and `read_pknca` require the `route` keyword: a batch has one route and the two formats carry none.
- Further changes of this branch which no 1.0.0 code could depend on: the per-interval variable of the value at the start of a dosing interval is `interval_c_start` (it is the post-dose value after a bolus), `EVID 4` and an `SS` value other than 0 or 1 raise in `read_events`, and a table without an `EVID` column reads a row with `AMT > 0` as a dose only.
