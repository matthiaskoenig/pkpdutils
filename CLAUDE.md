...
**`plot/` - figures.** `style.py` (`PlotStyle`, `dose_color`), `timecourse.py` (`plot_timecourse`, dose lines of a curve's protocol), `nca.py` (`plot_nca`, `plot_nca_grid`, `draw_nca_panel` shading `AUC(0-tau)` of a multiple dose result, `plot_intervals` of the `interval_*` variables against the interval number), `fit.py` (`plot_fit`, `plot_goodness_of_fit`, `plot_dose_proportionality`, `plot_bland_altman`), `parameters.py` (`plot_parameters`), `ratio.py` (`plot_ratio`), `meta.py` (`plot_forest`). Functions return the `Figure`, take `ax` (`plot_bland_altman` does not), never show.
